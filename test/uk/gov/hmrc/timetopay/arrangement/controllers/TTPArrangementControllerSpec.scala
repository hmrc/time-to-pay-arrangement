...
class TTPArrangementControllerSpec extends UnitSpec with MockFactory with ScalaFutures {
  ...
  "POST /ttparrangements" should {
    "return 201" in {
      val arrangementServiceStub = stub[TTPArrangementService]

      object TTPTestArrangementController extends TTPArrangementController {
        override val arrangementService = arrangementServiceStub
      }
      ...
      val result = TTPTestArrangementController.create().apply(fakeRequest).futureValue
  ...
  "GET /ttparrangements" should {
    "return 200 for arrangement" in {
      ...
      val result = TTPTestArrangementController.arrangement("XXX-XXX-XXX").apply(fakeRequest).futureValue
