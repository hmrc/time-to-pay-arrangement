...
object LetterAndControlBuilder {
  ...
  def create(ttpArrangement: SetupArrangementRequest): LetterAndControl = {
    ...
    val resolveCommsException: (Option[String], Option[String]) = {
      (for {
  ...
  private def resolveAddress(ttpArrangement: SetupArrangementRequest): AddressResult = {
    implicit val taxpayer: TaxpayerDetails = ttpArrangement.taxpayer
  ...
  private def multipleAddresses(implicit taxpayer: TaxpayerDetails): AddressResult = {
    val uniqueAddressTypes: List[JurisdictionType] = taxpayer.addresses.map(JurisdictionChecker.addressToJurisdictionType).distinct
