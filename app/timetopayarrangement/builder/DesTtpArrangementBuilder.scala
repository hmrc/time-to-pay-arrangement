...
import uk.gov.hmrc.timetopay.arrangement.{Instalment, Schedule, TaxpayerDetails}

object DesTtpArrangementBuilder {

  val formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy")

  def createDesTtpArrangement(setupArrangementRequest: SetupArrangementRequest): DesTtpArrangement = {
    ...
    model.DesTtpArrangement(
      ...
      enforcementAction    = enforcementFlag(setupArrangementRequest.taxpayer),
      debitDetails         = setupArrangementRequest.taxpayer.selfAssessment.debits.map { d => DesDebit(d.originCode, d.dueDate) },
