...
class ArrangementService @Inject() (
   ...
   */
  def submit(setupArrangementRequest: SetupArrangementRequest)(implicit hc: HeaderCarrier): Future[Unit] = {
    val desSetupArrangementRequest = DesSetupArrangementRequest(
      ttpArrangement   = DesTtpArrangementBuilder.createDesTtpArrangement(setupArrangementRequest),
      letterAndControl = LetterAndControlBuilder.create(setupArrangementRequest)
    )
    desConnector.submitArrangement(
      utr                        = setupArrangementRequest.taxpayer.selfAssessment.utr,
